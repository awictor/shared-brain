...
const heuristics: Heuristic[] = [
  { name: 'System Status', score: 5, description: 'System visibility of status for all operations', issue: 'Status: Fixed — Orange progress bar shows during all fetch requests; loading states visible throughout', recommendation: 'Loading indicator implemented via fetch monkey-patch in ux-enhance.js' },
  { name: 'Real-world Match', score: 5, description: 'Language matches user mental models', issue: 'Status: Fixed — Terminology is clear and intuitive throughout the interface', recommendation: 'Maintained current approach; language consistent across all views' },
  { name: 'User Control & Freedom', score: 5, description: 'Easy undo and escape from unwanted states', issue: 'Status: Fixed — 5-second undo toast on delete; confirmation modal for destructive actions', recommendation: 'Undo delete with countdown + confirmAction() modal implemented in ux-enhance.js' },
  { name: 'Consistency & Standards', score: 5, description: 'Follows platform conventions consistently', issue: 'Status: Fixed — Unified SPA with consistent component library; all demo pages consolidated', recommendation: 'All interfaces now follow the same design system and interaction patterns' },
  { name: 'Error Prevention', score: 5, description: 'Prevents errors before they occur', issue: 'Status: Fixed — Store button disabled when textarea empty; red border + validation message shown', recommendation: 'Client-side validation with disabled submit + visual feedback in ux-enhance.js' },
  { name: 'Recognition over Recall', score: 5, description: 'Minimize memory load with visible options', issue: 'Status: Fixed — Tag autocomplete fetches all existing tags on focus; dropdown with matches as user types', recommendation: 'Tag autocomplete with fetch-on-focus + type-ahead filtering in ux-enhance.js' },
  { name: 'Flexibility & Efficiency', score: 5, description: 'Accelerators for expert users', issue: 'Status: Fixed — Ctrl+? / Cmd+K shows command palette; 1-9 for page nav; all shortcuts discoverable', recommendation: 'Keyboard shortcut overlay + number-key navigation implemented in ux-enhance.js' },
  { name: 'Aesthetic & Minimal Design', score: 5, description: 'No irrelevant or rarely needed information', issue: 'Status: Fixed — Progressive disclosure via help panel; clean minimal interface with contextual info', recommendation: 'Help panel slides in only on demand; tour only shows on first run' },
  { name: 'Error Recovery', score: 5, description: 'Help users recognize and recover from errors', issue: 'Status: Fixed — Styled error toast with specific message + Retry button that re-sends the request', recommendation: 'Error recovery toast with actionable retry implemented in ux-enhance.js' },
  { name: 'Help & Documentation', score: 5, description: 'Accessible help and documentation', issue: 'Status: Fixed — Floating help button + slide-in panel + first-run tour with 4 guided steps', recommendation: 'Help panel, keyboard shortcuts section, quick guide, and onboarding tour in ux-enhance.js' },
];
