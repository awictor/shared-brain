...
describe('Multi-User Ownership Model', () => {
  ...
  beforeEach(() => {
    ...
    alice = new MemoryHandler(store, embeddings, vectorIndex, 'alice', 'Alice Smith');
    bob = new MemoryHandler(store, embeddings, vectorIndex, 'bob', 'Bob Jones');
  });
  ...
  describe('authorId always stamped from currentUserId', () => {
    ...
    it('stores correct authorId even after setCurrentUser is called', async () => {
      const handler = new MemoryHandler(store, embeddings, vectorIndex, 'charlie', 'Charlie');
